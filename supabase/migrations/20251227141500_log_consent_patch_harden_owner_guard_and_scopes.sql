-- Patch migration for `public.log_consent_if_allowed` after `20251226191139` was already applied.
--
-- Goals:
-- - Remove the `service_role` bypass: consent logging must run in end-user context only.
-- - Restore defensive scope normalization (filtering) while keeping the existing validation behavior.
-- - Revoke EXECUTE from `service_role` (least-privilege).
--
-- Context:
-- - The Edge Function `log_consent` derives `user_id` from `supabase.auth.getUser()` and calls this RPC.
-- - `service_role` must not be able to create consent records for arbitrary users.
...
begin
  ...
  if jsonb_typeof(p_scopes) = 'array' then
    ...
    -- Keep behavior stable: any unknown scope IDs are rejected.
    if exists (
      select 1
    ...
    -- Defensive filtering to keep normalization aligned with earlier migrations.
    normalized_scopes := (
    ...
    -- Keep behavior stable: unknown scope IDs are rejected (only for enabled scopes).
    if exists (
      select 1
    ...
    -- Filter: keep only enabled AND known scope IDs (defense-in-depth).
    normalized_scopes := (
