...
class CycleInfo {
  ...
  })  : assert(cycleLength >= 21 && cycleLength <= 60,
            'cycleLength should be between 21 and 60 days'),
        assert(periodDuration >= 1 && periodDuration <= 10,
            'periodDuration should be between 1 and 10 days') {
    if (cycleLength <= 0) {
      throw ArgumentError.value(
          cycleLength, 'cycleLength', 'must be positive');
    }
    if (periodDuration <= 0 || periodDuration > cycleLength) {
      throw ArgumentError.value(periodDuration, 'periodDuration',
  ...
  String phaseOn(DateTime d) {
    ...
    // Calculate day in cycle with positive modulo for negative diffs
