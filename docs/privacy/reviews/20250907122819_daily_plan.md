...
- **Vor Pflicht:** Backup-Verifikation (muss erfolgreich sein, bevor DROP):
  - Backup-Tabelle existiert (`public.daily_plan_backup`) und Row-Counts stimmen (z. B. `SELECT count(*)` auf beiden Tabellen).
  - Stichproben/Checksummen plausibel (z. B. Hash über IDs/created_at) oder DB-Dump Exit-Code/Restore-Test ist OK.
  - Falls `deleted_at` eingeführt wurde: prüfen, dass Soft-Deletes im Backup enthalten sind.
- Rücknahme durch `DROP TABLE public.daily_plan CASCADE;` (Achtung: irreversibler Datenverlust für alle `daily_plan`-Einträge; CASCADE entfernt abhängige Policies, Trigger und Indizes automatisch).
