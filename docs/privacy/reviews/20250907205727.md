...
## Zweck & Scope
Entfernt redundanten Trigger `trg_cycle_owner` (IaC-Hygiene). Keine neuen personenbezogenen Datenfelder; reine Bereinigung. `trg_cycle_owner` setzte `user_id := auth.uid()` bei INSERT; `set_cycle_data_user_id` übernimmt diese Ownership-Zuweisung bereits, daher geht keine Schutzwirkung verloren.
...
- Trigger wiederherstellen: `CREATE TRIGGER trg_cycle_owner BEFORE INSERT ON public.cycle_data FOR EACH ROW EXECUTE FUNCTION public.set_cycle_data_user_id();`
