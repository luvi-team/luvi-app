...
- Header: Begrüßung + Phase + Glocke + Week‑Strip (`lib/features/dashboard/screens/heute_screen.dart:214`).
- LUVI Sync Hero Preview (Bild, Yin‑Yang Badge, Teaser, CTA „Mehr“) – Key `dashboard_hero_sync_preview` (`lib/features/dashboard/screens/heute_screen.dart:134`).
- Kategorien Grid (4 Chips, dynamische Breiten, Wrap) – Key `dashboard_categories_grid` (`lib/features/dashboard/screens/heute_screen.dart:370`).
- „Deine Top‑Empfehlung“ – prominent (`lib/features/dashboard/screens/heute_screen.dart:160`, `lib/features/dashboard/widgets/top_recommendation_tile.dart:1`).
- „Weitere Trainings“ – horizontaler Scroller mit RecommendationCards – Key `dashboard_recommendations_list` (`lib/features/dashboard/screens/heute_screen.dart:426`).
- „Deine Trainingsdaten“ – StatsScroller (Wearable‑abhängig) – Key `dashboard_training_stats_scroller` (`lib/features/dashboard/screens/heute_screen.dart:182`, `lib/features/dashboard/widgets/stats_scroller.dart:1`).
- Info‑Karte zur Zyklusphase (CycleTipCard) (`lib/features/dashboard/screens/heute_screen.dart:356`).
- Bottom‑Dock + Floating Sync Button – Keys `dashboard_dock_nav`, `floating_sync_button` (`lib/features/dashboard/screens/heute_screen.dart:478`).

**Navigation/Routes**
- Screen‑Route: `HeuteScreen.routeName = '/heute'` (`lib/features/dashboard/screens/heute_screen.dart:22`).
...
- Floating Sync Button: `go('/luvi-sync')` + Tab aktiv (`lib/features/dashboard/screens/heute_screen.dart:498`).
...
- Empfehlungen: aktuell aus Fixtures; Filterung nach Kategorie ist vorbereitet (TODO‑Hook in `_onCategoryTap`) (`lib/features/dashboard/screens/heute_screen.dart:368`).
...
**Accessibility/Keys**
- Screen‑Anker: `dashboard_header`, `dashboard_hero_sync_preview`, `dashboard_categories_grid`, `dashboard_recommendations_list`, `dashboard_training_stats_scroller`, `dashboard_dock_nav`, `floating_sync_button` (`lib/features/dashboard/screens/heute_screen.dart:121`).
...
- Bild‑Jank – mitigiert durch `precacheImage` (Hero + TopReco) (`lib/features/dashboard/screens/heute_screen.dart:41`).
- Layout‑Enge – Category‑Breitenkompression (`lib/features/dashboard/utils/heute_layout_utils.dart:1`).
