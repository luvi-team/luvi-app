...
- Premium (IAP): Coach-Programme (LUVI-eigene Pläne), KI-Suche & KI-Playlists (semantisch, phasenbewusst), optional ad-light/ad-free im Feed (nie im YouTube-Player).
...
- Agenten-Governance & Wissen (neu): Archon (MCP) als zentrale, versionierte Wissensbasis (z. B. Phase-Definitionen, Consent-Texte, Ranking-Regeln); KI-Tools greifen konsistent darauf zu.
- LLM-Observability (neu): Langfuse protokolliert KI-Aufrufe (Prompts, Token/Kosten, Latenz), damit Qualität & Kosten im Betrieb sichtbar bleiben.
- Supabase MCP (Dev-only, read-only): Assistenten dürfen das Dev-Projekt lesen & Migrationspläne vorschlagen, aber nichts direkt in Prod schreiben. DB-Änderungen laufen ausschließlich über Migrations-PRs + CI-Gates (Dry-Run/RLS-Smoke) – so bleibt alles nachvollziehbar und sicher.

5. App Flow
Onboarding: Splash → Welcome → Consent (App) → Auth → Onboarding (Zyklus-Eckdaten, Ziele, Intensität, Alter, etc.) + Content-Präferenzen (Kategorien, Sprache) → Success.
Bottom-Nav:

Home · LUVI Stream · Coach · Puls (Zyklusdaten + Kalender & Trainingsleistung) · Einstellungen/Profil
